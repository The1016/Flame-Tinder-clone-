...
export default function SwipeDeck({
  ...
  const pages = useMemo(() => {
    ...
    for (const s of secs) {
      if (s.type === 'card') {
        const items: any[] = Array.isArray(s.content?.items) ? s.content.items : [];
  ...
  const doSwipe = useCallback(
    ...
    [current, token, cards.length, onSwiped, onEmpty]
  );
  ...
  }, [current, doSwipe, pageIndex, pages.length]);
  ...
  const renderSection = (c: Candidate, s: Section, itemIndex?: number) => {
        ...
        <TwoColCard
          ...
          right={
            <div className="space-y-3">
              ...
              <p className="text-neutral-300 whitespace-pre-wrap">{s.content?.body ?? ''}</p>
            </div>
          }
        />
      );
    ...
    if (s.type === 'gallery') {
      const imgs: string[] = Array.isArray(s.content?.images) ? s.content.images : [];
          ...
          right={
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {imgs.map((u, i) => (
                  <img key={i} src={u} alt="" className="w-full aspect-[4/3] object-cover rounded-lg border border-neutral-800" />
                ))}
              </div>
            </div>
          }
      ...
      <div className="grid h-full grid-cols-1 md:grid-cols-12 gap-4 md:gap-6">
        ...
        <div className="md:col-span-8 h-full">
          <div className="h-full w-full rounded-xl overflow-hidden border border-neutral-800 bg-neutral-900">
            <div className="h-full w-full md:aspect-[3/4]">
